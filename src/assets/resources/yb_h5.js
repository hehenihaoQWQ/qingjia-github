 ...
 */
function yibanhtml5location(position) {
    var obj = JSON.parse(position);
    var address = obj.address;
    address = address.replace(/\s+/g, "");
    if (address === "" || address === "null" || address === null || typeof (address) == "undefined") {
        $("input[name='position']").attr({"value": "定位失败请刷新重试"});
    } else {
        //赋值给位置文本框
        $("input[name='position']").val(address);
    }
}
